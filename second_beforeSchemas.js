...
// testing how do adjust and parse scores
fastify.post('/update-score/:id', async (request, reply) => {
	const userId = request.params.id;
	const { score } = request.body;

	// score is not a number before accessing database
	// Fastify can validate inputs automatically using JSON Schema instead of typeof checks like below
	if (typeof score != "number" || isNaN(score)) {
		 reply.code(400).send({ error: 'Invalid score value, must be a number'});
		 return;
	}
