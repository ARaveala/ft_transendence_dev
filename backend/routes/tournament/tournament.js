...
// 			flog.debug({ function: 'createTournament', body: request.body }, 'request body:');
 			try{
				...
				reply.code(400).send({status: 'ERROR', message: "error in create tournament"});
 			}
 		}
 	});
}
        ...
        schema: tournamentSchema,
 		handler: async (request, reply) => {
			...
			try {
				...
				reply.code(401).send({ status: 'ERROR', error: 'Verification failed?' });
			}
 		}
 	});
}
 	...
 	fastify.route({
		...
		// Round 1: seed 1 vs seed 4
			...
			//	flog.error({fucntion: 'startTournament', errStack: err.stack, errMessage: err.message}, "error :: in start tournament"); //wrong
				reply.code(400).send({ status: 'ERROR', error: 'Start tournament failed?' });//wrong	
			}
		}
		...
		url: API_PROTOCOL.REMOVE_PLAYER_FROM_TOURNAMENT.path,
 		handler: async (request, reply) => {
			...
			try {
				...
				reply.code(400).send({status: 'ERROR', error: "error removing from tournamnet"});
			}
		}
	})
}
		...
		url: API_PROTOCOL.CANCEL_TOURNAMENT.path,
 		handler: async (request, reply) => {
			...
			try {
				...
				reply.code(400).send({status: 'ERROR', error: "error canceling tournamnet"});
			}
		}
	})
}
